...
const applyDict = (dict: DictValue, args: Value[]): Result<Value> => {
    if (args.length !== 1) {
        return makeFailure(`Dictionary lookup expects exactly one argument, got ${args.length}`);
    }
    const key = args[0];
    if (!isSymbolSExp(key)) {
        return makeFailure(`Dictionary key must be a symbol, got ${format(key)}`);
    }
    const entry = dict.entries.find(([k]) => k.val === key.val);
    return entry ? makeOk(entry[1]) : makeFailure(`Key ${key.val} not found in dictionary`);
};
