...
;; <entry> ::= (<cexp> <cexp>)              / Entry(key: symbol, value: CExp)
...
// L3
export type Entry = {tag: "Entry"; key: SymbolSExp; value: CExp; }
export type LitExp = {tag: "LitExp"; val: SExpValue; }
export type DictExp = {tag: "DictExp"; entries: Entry[]; }
...
export const makeLitExp = (val: SExpValue): LitExp =>
    ({tag: "LitExp", val: val});
export const makeEntry = (key: string, value: CExp): Entry =>
    ({tag: "Entry", key: makeSymbolSExp(key), value: value});
export const makeDictExp = (entries: Entry[]): DictExp =>
...
const parseDictExp = (params: Sexp[]): Result<DictExp> => {
    if (!isGoodEntries(params)) {
        return makeFailure('Malformed entry in "dict" expression');
    }
    ...
    for (const entry of params) {
        ...
        if (keys.has(key)) {
            return makeFailure(`Duplicate key in dictionary: ${key}`);
        }
        keys.add(key);
    }

    return mapv(mapResult((entry: Sexp) => 
        bind(parseL32CExp(entry[1]), (value: CExp) => 
            makeOk(makeEntry(entry[0].toString(), value))), params), 
        (entries: Entry[]) => makeDictExp(entries));
};

const isGoodEntries = (entries: Sexp): entries is [string, Sexp][] =>
    isArray(entries) &&
    allT<NonEmptyList<Sexp>>(isNonEmptyList, entries) &&
    allT(isIdentifier, map(first, entries));
...
const unparseDictExp = (de: DictExp): string =>
    `(dict ${map((e: Entry) => `(${valueToString(e.key)} ${format(e.value)})`, de.entries).join(" ")})`;
